<template>
    <div class="wrapper" @viewappear="viewappear" @viewdisappear="viewdisappear">
        ...
        <div class="footer">
            ...
            <div class=" enterButton">
                <wxc-button class="okButton" text="下一步" :textStyle="GLOBAL.buttonText" :btnStyle="GLOBAL.smDefaultButton"
...
<script>
    ...
    export default {
        ...
        // 注册按钮组件
        components: {WxcButton,WxcMinibar,Toast},
        created(){
            try {
            ...
            post.fetch({
                ...
                if(res.data.res === 1) {
                    ...
                    if (invoice.type == '单位') {
                        that.invoiceName = invoice.trueName
                        that.dutyParagraph = invoice.dutyNum
                        that.companyPhoneNumber = invoice.comTel
                        that.companyEMail = invoice.email
                        that.companyAddress = invoice.comAddress
                        that.depositBank = invoice.bank
                        that.bankAccount = invoice.bankNo
                        that.type = invoice.type
                    }else{
                        that.toggle('personal')
                        that.invoiceName = invoice.trueName
                        that.phoneNumber = invoice.phone
                        that.address = invoice.address
                        that.eMail = invoice.email
                    }
                }
            });
        },
        ...
        methods: {
            ...
            wxcButtonClicked() {
                ...
                // 表单信息验证
                if(this.invoiceName == ''){
                    toast.editContent("抬头名称不能为空");
                    return false;
                }
                if(this.show){
                    if (!(/^[A-Z0-9]{15}$|^[A-Z0-9]{17}$|^[A-Z0-9]{18}$|^[A-Z0-9]{20}$/.test(this.dutyParagraph))) {
                        toast.editContent("请正确填写税号");
                        return false;
                    }
                    if (!(/^(0\d{2,3}-?)?\d{7,8}$/.test(this.companyPhoneNumber))) {
                        toast.editContent("请正确填写单位电话");
                        return false;
                    }
                    if (!(/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/.test(this.companyEMail))){
                        toast.editContent("请正确填写邮箱");
                        return false;
                    }
                    if (!(/^(\d{16}|\d{19})$/.test(this.bankAccount))) {
                        toast.editContent("请正确填写银行账号");
                        return false
                    }
                ...
                // 保存成功后储存缓存
                var invoice = JSON.stringify({trueName:this.invoiceName,dutyNum:this.dutyParagraph,comTel:this.companyPhoneNumber,
                    email:email,comAddress:this.companyAddress,bank:this.depositBank,bankNo:this.bankAccount,
                    phone:this.phoneNumber,address:this.address,type:this.type});
                ...
                post.fetch({
                    ...
                    body: param,
                    headers: { "Content-Type": "application/x-www-form-urlencoded" },
                    url: 'https://www.aftdc.com/businessapp/Recovery/setInvoice',
                }, function (res) {
                    if(res.data.res === 1) {
                        if (that.needBack==1) {
