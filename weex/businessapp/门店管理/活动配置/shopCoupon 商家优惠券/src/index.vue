<template>
    <div class="wrapper" @touchstart="loseBlur">
        ...
        <div class="minibar">
            ...
            >
                ...
                <text :style="GLOBAL.headerCenter"  slot="middle">商家优惠券</text>
            </wxc-minibar>
        </div>
        ...
        <scroller class="content">
            <div class="contentClassfly">
                <div class="betweenDiv">
                    <text :style="{fontSize:GLOBAL.bigFS,color:GLOBAL.dColor}">活动名称</text>
                    <div class="inputDiv">
                        <input class="input" ref="firInput" @focus="allFocus" @blur="allBlur"  v-model="couponName" type="text" placeholder="请输入活动名称（限10字以内）" :style="GLOBAL.inputText"/>
            ...
            <div class="contentClassfly">
                <div class="betweenDiv">
                    <text :style="{fontSize:GLOBAL.bigFS,color:GLOBAL.dColor}">优惠卷金额</text>
                    <div class="inputDiv">
                        <input class="input" ref="secInput" @focus="allFocus" @blur="allBlur"  v-model="couponMoney" type="text" placeholder="请输入优惠卷金额" :style="GLOBAL.inputText" style="margin-left: 70px"/>
                ...
                <div class="betweenDiv">
                    ...
                    <div class="inputDiv">
                        ...
                        <input class="input inputWidth" ref="thrInput" @focus="allFocus" @blur="allBlur" :style="GLOBAL.inputText" type="text" style="margin-left: 10px" v-model="spendMoney" placeholder="请输入金额" />
                        <text :style="{fontSize:GLOBAL.bigFS,color:GLOBAL.dColor}">元</text>
                    </div>
                </div>
                <div class="betweenDiv">
                    <text :style="{fontSize:GLOBAL.bigFS,color:GLOBAL.dColor}">发放渠道</text>
                    <div class="inputDiv">
                        <div class="radio_content" @click="radioClick(0)">
                            <image class="imgDefault" v-if="radio==1"
                                   src="https://image.aftdc.com/appBimg/radio.png"></image>
                            <image class="imgDefault" v-if="radio==0"
                                   src="https://image.aftdc.com/appBimg/radio_check.png"></image>
                            <text class="radioNameSize" :style="{fontSize:GLOBAL.bigFS,color:GLOBAL.dColor}">到店领取</text>
                        </div>
                        <div class="radio_content" @click="radioClick(1)">
                            ...
                            <image class="imgDefault" v-if="radio==1"
                                   src="https://image.aftdc.com/appBimg/radio_check.png"></image>
                            <text class="radioNameSize" :style="{fontSize:GLOBAL.bigFS,color:GLOBAL.dColor}">分享领取</text>
                        </div>
        ...
        methods: {
            ...
            save:function(){
                ...
                if (Number(this.spendMoney) < Number(this.couponMoney)) {
                    toast.editContent('优惠券金额大于使用条件金额');
                    return false
                }
                if(!this.isOn) {
                ...
                var that = this;
                param+= '&couponName='+that.couponName;
                param+= '&timez='+that.startTime;
                param+= '&timey='+that.endTime;
                param+= '&spendMoney='+that.spendMoney;
                param+= '&couponMoney='+that.couponMoney;
                param+= '&couType='+that.couType;
                ...
                post.fetch({
                    ...
                    url: 'https://www.aftdc.com/businessapp/Activity/addCoupon',
                }, function (e) {
                    if (e.data.res == -2) {
                        //跳到登陆页面
                        navigator.push({
