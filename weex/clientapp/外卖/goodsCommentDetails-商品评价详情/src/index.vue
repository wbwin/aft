<template>
    <div class="reply"  @touchstart="blurClick">
        ...
        <wxc-minibar
            ...
            <text :style="GLOBAL.headerCenter"  slot="middle">{{comment.reply_num > 0 ? comment.reply_num + '条评论' : '暂无评论'}}</text>
        </wxc-minibar>
        ...
        <scroller class="scroller" show-scrollbar="false">
            <div class="mySelected line">
                ...
                <div class="pinLun_foodImg" v-if="comment.appraisesAnnex!=[]">
                    <image class="ratGoodImg" v-for="(img,imgIndex) in comment.appraisesAnnex" @click="onEnlargeImageClick(index,imgIndex)"  :src="'https://image.aftdc.com/'+img"></image>
                </div>
            ...
            <div class="mySelected" v-for="(item,index) in reply"  :key="" @click="leavingClick(index)">
                ...
                <div class="buttom aiCenter">
                    ...
                    <text :style="{fontSize:'22px',color:GLOBAL.dColor}">&nbsp;·&nbsp;</text>
                    <text class="report" :style="{fontSize:'22px',color:GLOBAL.dColor}">评论</text>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            this.id= JSON.parse(weex.config.param).id
        ...
        methods:{
            ...
            deleteComment:GLOBAL.throttle(function() { //删除评论或回复
                ...
                if(that.commentIndex === -1) {
                    ...
                    url = 'https://www.aftdc.com/userapp/Mine/delGoodsZP'
                }
            ...
            getZP(page) { //获取全部回复
                ...
                post.fetch({
                    ...
                    body: param,
                    headers: { "Content-Type": "application/x-www-form-urlencoded" },
                    url: 'https://www.aftdc.com/userapp/Mine/getGoodsZP',
                }, function (res) {
                    console.log(param)
                    console.log(JSON.stringify(res))
                    if(res.data.res === 1) {
            ...
            sendClick:GLOBAL.throttle(function() { //发布回复与评论回复
                ...
                if(that.replyIndex === -1) {
                    url = 'https://www.aftdc.com/userapp/Mine/goodsZP'
                }else {
                    url = 'https://www.aftdc.com/userapp/Mine/goodsReply'
                    param += '&replyId=' + that.reply[that.replyIndex].id
                    if(that.reply[that.replyIndex].userId){
                        var replyInfo = {userId:that.reply[that.replyIndex].userId,userName:that.reply[that.replyIndex].userName,content:that.reply[that.replyIndex].content}
                    }else{
                        var replyInfo = {shopId:that.reply[that.replyIndex].shopId,userName:that.reply[that.replyIndex].userName,content:that.reply[that.replyIndex].content}
                    }
            ...
            commentFabulous:GLOBAL.throttle(function(item,type){ //点赞评论
                ...
                if(type === 0) {
                    if(item.isPraise === 1) {
                        url = 'https://www.aftdc.com/userapp/Index/cancelZanPj' //取消点赞评论
                    }else {
                        url = 'https://www.aftdc.com/userapp/Index/zanPj' //点赞评论
                    }
                }else {
                    if(item.isPraise === 1) {
                        url = 'https://www.aftdc.com/userapp/Mine/cancelPraiseGoodsZP' //取消点赞回复
                    }else {
                        url = 'https://www.aftdc.com/userapp/Mine/praiseGoodsZP' //点赞回复
                    }
                }
