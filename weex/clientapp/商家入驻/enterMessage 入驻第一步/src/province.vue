        ...
        <wxc-popup ref="wxcPopup"
            ...
            <div class="demo-content">
                <div class="popupButton">
                    <text class="defaultSize cancelText" @click="cancelClick()">取消</text>
                    <text class="defaultSize sureText" @click="sureClick()">确定</text>
                </div>
                <div class="popupScroller">
                    <scroller class="scrollerDiv" show-scrollbar="false" >
                        <text class="defaultSize provinceText" @click="provinceListClick(index)" :class="[provinceListIndex==index?'provinceChoice':'']" v-for="(item,index) in provinceList">{{item.name}}</text>
                    </scroller>
                    <scroller class="scrollerDiv" show-scrollbar="false"   >
                        <div :class="[provinceList[provinceListIndex].cityList.length<3?'scrollerCenter':'']" >
                        <text class="defaultSize provinceText" @click="cityListClick(index)" :class="[cityListIndex==index?'provinceChoice':'']" v-for="(item,index) in provinceList[provinceListIndex].cityList">{{item.name}}</text>
                        </div>
                    </scroller>
                    <scroller class="scrollerDiv" show-scrollbar="false"  >
                        <div :class="[provinceList[provinceListIndex].cityList[cityListIndex].areaList.length<3?'scrollerCenter':'']">
                        <text class="defaultSize provinceText" @click="areaListClick(index)" :class="[areaListIndex==index?'provinceChoice':'']" v-for="(item,index) in provinceList[provinceListIndex].cityList[cityListIndex].areaList">{{item}}</text>
                        </div>
                    </scroller>
                </div>
            </div>
...
<!--<province ref="province" v-on:sureClick='getProvinceValue'></province>//父组件引用此组件需在HTML里添加此代码-->
<!--getProvinceValue(provinceValue) {//父组件引用此组件需在methods里添加此代码-->
        <!--var that=this-->
        <!--that.provinceValue = provinceValue //that.provinceValue为父组件里data的数据，选择完地址点击确定后数据会变为类似‘北京市-北京市-东城区’-->
<!--},-->
<script>
    import {WxcMinibar, WxcButton, WxcIcon, WxcMask, WxcPopup} from 'weex-ui';
    export default {
        components: {WxcMinibar, WxcButton, WxcIcon, WxcMask, WxcPopup},
        data () {
            return {
                isBottomShow: false,
                provinceListIndex:0,
        ...
        methods: {
            provinceListClick(index){
                var that=this
                that.provinceListIndex=index
                that.cityListIndex=0;
                that.areaListIndex=0
            },
            cityListClick(index){
                var that=this
                that.cityListIndex=index
                that.areaListIndex=0
            },
            areaListClick(index){
                var that=this
                that.areaListIndex=index
            },
            provinceShow(){//父组件调用此方法显示地址选择弹层   const province = this.$refs.province;province.provinceShow();
              var that=this
            ...
            sureClick(){
                var that=this
                that.provinceValue=that.provinceList[that.provinceListIndex].name+'-'+that.provinceList[that.provinceListIndex].cityList[that.cityListIndex].name+'-'+that.provinceList[that.provinceListIndex].cityList[that.cityListIndex].areaList[that.areaListIndex]
                that.$emit('sureClick', that.provinceValue)//用于把数据传回给父组件，父组件用v-on监听sureClick事件即可获取数据
