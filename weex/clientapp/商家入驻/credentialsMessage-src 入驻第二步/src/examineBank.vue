...
<script>
    export default {
        ...
        methods: {
            getBankInfoByCardNo(cardNo){//用于返回所输入的卡号相对应的银行
                var that=this
                for (var i = 0, len = that.bankcardList.length; i < len; i++) {
                    var bankcard = that.bankcardList[i];
                    var patterns = bankcard.patterns;
                    for (var j = 0, jLen = patterns.length; j < jLen; j++) {
                        var pattern = patterns[j];
                        if ((new RegExp(pattern.reg)).test(cardNo)) {
                            var info = that.extend(bankcard, pattern);
                            delete info.patterns;
                            delete info.reg;
                            info['cardTypeName'] = that.getCardTypeName(info['cardType']);
                            return info;//返回银行卡结果
                        }
                    }
                }
            ...
            getCardTypeName(cardType){
                var that=this
                if (that.cardTypeMap[cardType]) {
                    return that.cardTypeMap[cardType]
                }
                return undefined;
            },
            ...
            getbankNameAndCardNo(bankName,cardNo){//用于判断所选的银行和卡号是否匹配
                var that=this
                var bankcardList=that.bankcardList
                for (var i = 0, len = bankcardList.length; i < len; i++) {
