        ...
        <list class="scroller" show-scrollbar="false">
            ...
            <cell class="cell" ref="shopDel">
                <div class="newAllDiv" v-for="(item,index) in messageInfo" :key="index">
                    ...
                    <div class="row newAll">
                        <image class="useImg" :src="item.img" @click="personeHomePage(index)"></image>
                        <div>
                            <div class="row name">
                                <text :style="{fontSize:GLOBAL.bigFS,color:GLOBAL.dColor}" @click="personeHomePage(index)">{{item.name}}</text>
                                <text :style="{fontSize:GLOBAL.bigFS,color:GLOBAL.dColor}" @click="classification(index,item.type)">{{item.msg}}</text>
                            </div>
                            <text :style="{fontSize:GLOBAL.defaultFS,color:'#999'}" @click="personeHomePage(index)">{{item.createTime}}</text>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            allDel:GLOBAL.throttle(function () {//执行删除
                var that=this;
                let num=0;
                var delArr= [];
                for(var i in that.messageLength){
                ...
                post.fetch({
                    ...
                    type: 'json',
                    body: param,
                    url: 'https://www.aftdc.com/userapp/Mine/delMessage',
            ...
            getNews(index){//获取店铺数据
                ...
                post.fetch({
                    ...
                    body: param,
                    headers: { "Content-Type": "application/x-www-form-urlencoded" },
                    url: 'https://www.aftdc.com/userapp/Mine/getMessage',
                }, function (res) {
                    if(res.data.res==1){
            ...
            onrefresh:GLOBAL.throttle(function () {//店铺下拉刷新
                var that=this;
            ...
            onloading:GLOBAL.throttle(function () {//店铺上拉加载
                var that=this;
            ...
            personeHomePage:GLOBAL.throttle(function (index) {//跳转至个人主页
                var that=this;
                var id=that.messageInfo[index].id;
                var url='http://assets/personalHomepage.js';
                var param=id;
                event.toUrl({url:url,param:param});
            }),

            classification:GLOBAL.throttle(function (index,type) {//评论详情页、留言详情页、评价详情页
                var that=this;
                if(type==1){//评价详情页
                }
                else if(type==2){//评论详情页
                    var id=that.messageInfo[index].typeId;
                    var url='http://assets/commentDetails.js';
                    var param=id;
                    event.toUrl({url:url,param:param});
                }
                else if(type==3){//留言详情页
