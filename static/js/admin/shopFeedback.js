        ...
        var app = new Vue({
            ...
            methods: {
                ...
                pageFn: function (pageCurent) {
                    ...
                    axios({
                        method: "POST",
                        url: "https://www.aftdc.com/aftdcAControl/Feedback/shopFeedback",
                        data: {
                            page: page,
                            login_name: that.loginName,
                        ...
                        if (require.res === 1) {
                            for (var i=0;i<require.data.length;i++) {
                                that.temp.push(that.cloneObj(require.data[i]))
                            }
                            that.listData = require.data;
                            for (var i = 0; i < that.listData.length; i++) {
                                if (that.listData[i].content)
                                    that.listData[i].content = that.listData[i].content.substr(0, 15) + '...';
                                if (that.listData[i].adminReply)
                                    that.listData[i].adminReply = that.listData[i].adminReply.substr(0, 15) + '...';
                ...
                reply: function (id, index) {
                    ...
                    location.href = "./shopFeedbackDetail.html";
                },
                ...
                del: function (id, index) {
                    ...
                    if (confirm("确认删除该提议？")) {
                        axios({
                            method: "post",
                            url: 'https://www.aftdc.com/aftdcAControl/Feedback/shopdel',
                            data: {
                                feedbackId: id,
                                adminNum: adminData.adminNum,
                                token: adminData.token
                ...
                choiceAll: function ()
                {
                    ...
                    if (!event.currentTarget.checked) {//实现反选
                        ...
                        _this.listData.forEach(function(item) {
                            _this.checkboxModel.push(item.feedbackId);
                        });
                    }
