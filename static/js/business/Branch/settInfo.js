...
var app = new Vue({
    ...
    methods: {
        ...
        onloads: function() {
            ...
            } catch (err) {　　　　 console.log(err, '经纬度错误')　　 }
            axios({
                methods: "post",
        ...
        formSubmit: function() {
            ...
            var unm = 0; //上传数量
            var five = 3; //实际
            var door_img = this.shopPhoto[0].upImg; //们年
            var okimgzore = this.shopPhoto[0].okimg; //logo
            if (okimgzore == '') {
                alert('请上传门脸图')
                return false
            }
            if (okimgzore.substr(0, 5) == "Public") {
                data.door_img = okimgzore;
                five--
            }
            var store_img = this.shopPhoto[1].upImg; //店内okimg
            var okimgone = this.shopPhoto[1].okimg; //logo
            if (okimgone == '') {
                alert('请上传店内图')
                return false
            }
            if (okimgone.substr(0, 5) == "Public") {
                data.store_img = okimgone;
                five--
            }
            var okimglogin = this.shopPhoto[2].okimg; //logo
            var shopImg = this.shopPhoto[2].upImg; //logo
            if (shopImg == '') {
                alert('请上传logo图')
                return false
            }
            if (okimglogin.substr(0, 5) == "Public") {
            ...
            axios({
                methods: "post",
                url: '/index.php/Business/Branch/new_in',
                data: data,
            }).then(function(res) {
                if (res.res == 1) {
                    var shopId = res.id
                    if (five == 0) {
                        ...
                        window.parent.location.hash = "#/Branch/apti/1"
