...
$.fn.dateSelect = function(options) {
	...
	var newDataY,newDataM,newDataD
    ...
    (function(){
    	var data=new Date()
    	newDataM = data.getMonth() + 1;
    	newDataY = data.getFullYear();   	
    	newDataD = data.getDate();
    })();
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	if (typeof(evaluate) !== "undefined") {
        transferTimeFn(evaluate);
	}
	if (typeof(app) !== "undefined") {
        transferTimeFn(app);
	}
	//调用 timeFn 方法
	function transferTimeFn(pageObj) {
        try{
            var name=$(obj).attr('name');
            pageObj.$set(pageObj.time,name,date_sel);
            pageObj.timeFn()
        }catch(e){
            console.log(pageObj + '--err')
        }
	}
  
    	//evaluate.time[name]=date_sel
...
//------------------------------------------------------------------------------------------------------------------------  --------	
    ...
    $("#datepicker_" + elem_id+" .right").click(function(){
    				//newDataY,newDataM,newDataD 	
    			if(tz_y==newDataY && tz_m==newDataM){
    				return false
    			}
    ...
    {	
    	...
    	for(var i=1;i<=days;i++)
    	{
    		...
    		//---
    		if(newDataY==tz_y && newDataM==tz_m){
    			if(i>newDataD){
    				a_str ="disabled";
    			}	
    		}
    		//选择结束日期时，要拿开始日期判断，结束日期不能小于开始日期
    		var name=$(obj).attr("name");
    		if(name=="end"){
    			var start=$("#date").val();
    			var d1=(new Date(start.replace(/-/g,"\/")));
    			var startY=d1.getFullYear()
    			var startM = d1.getMonth() + 1;
		    	var startD = d1.getDate();
		    if(startY>tz_y){//开始的年大于显示的年
	    	 	a_str ="disabled";
	    	 }else if(startY==tz_y && startM > tz_m){//当开始的年等于显示的年，开始的月小于显示的月
	    	 	a_str ="disabled";
	    	 }else if(startY==tz_y && startM == tz_m && startD>i){///当开始的年等于显示的年，月等于月，开始日小于显示日
	    	 	a_str ="disabled";
	    	 }
    			...
    			var endY=d2.getFullYear()
    			var endM = d2.getMonth() + 1;
		    	var endD = d2.getDate();
		    if(endY<tz_y){
	    	 	a_str ="disabled";
	    	 }else if(endY==tz_y && endM < tz_m){
	    	 	a_str ="disabled";
	    	 }else if(endY==tz_y && endM == tz_m && endD<i){
	    	 	a_str ="disabled";
	    	 }
    		}
        ...
        var wid=$("#datepicker_" + elem_id).width()*14.285/100;
        $("#datepicker_" + elem_id +" .day li").first().css({"margin-left":wid*dayOfWeek});
    ...
    $(document).bind("click", function(event) {
       ...
       if (elem.id == "datepicker_" + elem_id || $(elem).parent()[0].id == "datepicker_" + elem_id || ($(elem).parent().parent()[0]&&$(elem).parent().parent()[0].id == "datepicker_" + elem_id)) {
          return;
       } 
       else if(elem.id == "ym"+elem_id || $(elem).parent()[0].id == "ym"+elem_id || ($(elem).parent().parent()[0] && $(elem).parent().parent()[0].id == "ym"+elem_id))
       {
    	   return;
       }
       else if(elem.id == "mlist_"+elem_id || $(elem).parent()[0].id == "mlist_"+elem_id || ($(elem).parent().parent()[0]&& $(elem).parent().parent()[0].id == "mlist_"+elem_id))
       {
    	   return;
       }
       else if(elem.id == "ylist_"+elem_id || $(elem).parent()[0].id == "ylist_"+elem_id ||($(elem).parent().parent()[0]&&  $(elem).parent().parent()[0].id == "ylist_"+elem_id))
       {
    	   return;
       }
       else {
