    ...
    for (var i = 0, len = bankcardList.length; i < len; i++) {
        ...
        for (var j = 0, jLen = patterns.length; j < jLen; j++) {
            ...
            if ((new RegExp(pattern.reg)).test(cardNo)) {
                var info = this.extend(bankcard, pattern);
...
function getCardTypeName(cardType) {
    if (cardTypeMap[0][cardType]) {
        return cardTypeMap[0][cardType]
    }
    return undefined;
}
...
//用于判断所选的银行和卡号是否匹配
function getbankNameAndCardNo(bankName, cardNo) {
    for (var i = 0, len = bankcardList.length; i < len; i++) {
        if (bankcardList[i].bankName == bankName) {
...
module.exports = {
  getbankNameAndCardNo: getbankNameAndCardNo
}
